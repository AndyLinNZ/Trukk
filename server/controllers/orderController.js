...
const updateOrderCancelled = async (req, res, next) => {
  try {
    ...
    io.sockets
      .to(updatedOrder.vendorId.toString())
      .emit("new-cancelled-order");
    res.status(200).json({
      success: orderStatus.nModified ? true : false,
      data: updatedOrder,
    });
...
const getCustomerExistingOrder = async (req, res, next) => {
  try {
    ...
    // get vendor info and name
    const vanInfo = await Van.findOne({
      _id: outstandingOrder.vendorId.toString(),
    }).lean();

    // get snack info and name
    const allSnackInfo = await Promise.all(
      outstandingOrder.snacks.map(async (snack) => {
        const snackInfo = await Snack.findById(snack.snackId.toString()).lean();
        return { ...snack, name: snackInfo.name, id: snackInfo._id, price: snackInfo.price };
      })
    );
    res.status(200).json({
      success: true,
      data: { ...outstandingOrder, van: vanInfo, snacks: allSnackInfo }
    });
  } catch (err) {
    console.log(err);
    next(err);
